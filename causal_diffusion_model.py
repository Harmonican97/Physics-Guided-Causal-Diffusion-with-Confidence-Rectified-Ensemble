...
class Block1D(nn.Module):
    def __init__(self, in_ch, out_ch, time_emb_dim, num_fault_components):
        ...
        # 【改进点】：条件映射不再是 Embedding，而是线性层，接受 Multi-hot 向量
...
class CausalUNet1D(nn.Module):
    def __init__(self, input_dim=1, base_channels=64, channel_mults=(1, 2, 4, 8), num_fault_components=3):
        ...
        # 【核心改进】：使用 Linear 替代 Embedding 以支持 Multi-hot 叠加
        # 输入: (B, 3) -> 输出: (B, time_emb_dim)
    ...
    def forward(self, x, t, y_multihot):
        ...
        c_emb = self.class_encoder(y_multihot) # 编码组合特征
...
# ==========================================
# 3. 物理机理损失 (Multi-Frequency)
# ==========================================
# class PhysicsLoss:
#     @staticmethod
#     def envelope_spectrum_loss(signal, target_freqs_list, fs=20480):
#         """
#         改进：同时最大化多个频率点的能量
#         target_freqs_list: List of lists. 每个样本对应一个频率列表。
#         """
#         B, C, L = signal.shape
...
#         for i in range(B):
#             freqs = target_freqs_list[i] # 这是一个列表，例如 [120, 180, 90]
#             # 过滤掉 0 Hz (无故障情况)
#             freqs = [f for f in freqs if f > 1.0]
            
#             if not freqs:
#                 continue # 如果是正常样本，不计算 Physics Loss
            
#             sample_loss = 0
#             total_energy = torch.sum(fft_mag[i, 0, :]) + 1e-8
            
#             for f_target in freqs:
#                 idx = (torch.abs(freq_axis - f_target)).argmin()
#                 # 扩大窗口以容忍频率泄露
#                 start = max(0, idx - 4)
#                 end = min(fft_mag.shape[-1], idx + 5)
#                 target_energy = torch.sum(fft_mag[i, 0, start:end])
                
#                 # 我们希望每个故障频率的能量占比都高
#                 # Log-Sum-Exp 或者是简单的 Sum-Log
#                 sample_loss += -torch.log(target_energy / total_energy + 1e-8)
            
#             loss += sample_loss / len(freqs) # 平均多频点的 Loss
#             valid_samples += 1
...
# causal_diffusion_model.py (部分更新)

class PhysicsLoss:
    @staticmethod
    def envelope_spectrum_loss(signal, target_freqs_list, fs=20480, harmonics=3):
        """
        [改进版] 支持谐波 (Harmonics) 引导
        harmonics: 考虑多少阶谐波 (默认3: 基频, 2倍频, 3倍频)
        """
        B, C, L = signal.shape
        envelope = torch.abs(signal)
        fft_res = torch.fft.rfft(envelope, dim=-1, norm='ortho')
        ...
        for i in range(B):
            ...
            for f_base in freqs:
                ...
                for k in range(1, harmonics + 1):
                    ...
                    # [关键改进] 使用更宽的窗口 (Window=5 -> 7)，容忍转速波动
                    start = max(0, idx - 6)
...
class PhysicsGuidedDiffusion(nn.Module):
    ...
    def physics_guided_sample(self, shape, target_multihot, target_freqs_list, guidance_scale=10.0):
        ...
        # 预先处理好 freqs，避免循环中重复计算
        # target_freqs_list 直接传入 PhysicsLoss
        ...
        for i in reversed(range(0, self.timesteps)):
            ...
            # 动态调整引导力度：在生成初期(噪声大)力度小，后期(信号清晰)力度大
            # 或者保持恒定。这里保持恒定简单有效，但稍微减小 scale
    ...
    def physics_guided_sample_from_t(self, x_t, t_start, target_multihot, target_freqs_list, guidance_scale=10.0):
        """
        guidance_scale: 可以是 float，也可以是 shape 为 (B, 1, 1) 的 Tensor
        """
        img = x_t
